...
/**
 * 🔄 带自动备用的图片生成请求
 * ✅ 顺序降级：先请求第1个端点，成功立即返回；失败才尝试下一个，避免重复计费
 */
async function fetchWithFallback(requestBody, isGemini3Native = false) {
    ...
    // ✅ 顺序降级：逐个端点尝试，成功立即返回，避免并行重复扣费
    for (let idx = 0; idx < YUNMENG_ENDPOINTS.length; idx++) {
        const endpoint = YUNMENG_ENDPOINTS[idx];
        const url = `${endpoint}${apiPath}`;
        console.log(`[banana2] ☁️ 尝试端点 ${idx + 1}/${YUNMENG_ENDPOINTS.length}: ${endpoint}`);
        try {
            ...
            // 4xx 客户端错误（非429）不再尝试其他端点，直接抛出
            if (response.status >= 400 && response.status < 500 && response.status !== 429) {
                const has429 = errors.some(e => e.status === 429);
                if (!has429) throw new Error(`请求失败: ${response.status}`);
            }
        } catch (err) {
            if (err.message?.startsWith('请求失败:') || err.message?.includes('参数') || err.message?.includes('审核')) {
                throw err; // 业务错误直接抛出，不重试
            }
    ...
    // 所有端点均失败
    const has429 = errors.some(e => e.status === 429);
    if (has429) throw new Error('请求过于频繁，请稍后重试');
    throw new Error(`云梦API节点均不可访问: ${errors.map(e => e.error || e.status).join(', ')}`);
}
