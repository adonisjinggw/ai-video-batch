...
(function (global) {
    ...
    async function callZhenzhenTextAPI(prompt, options = {}) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                lastErr = fetchErr;
                // 🔧 ERR_CONNECTION_CLOSED：后端已处理并扣费，不重试
                if (/ERR_CONNECTION_CLOSED/i.test(fetchErr.message)) {
                    console.warn(`[ZhenzhenText] 连接断开(后端已处理)，跳过重试`, fetchErr.message);
    ...
    async function callWriterLLM(messages, opts = {}) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                // 🔧 ERR_CONNECTION_CLOSED：后端已处理并扣费，不重试
                if (/ERR_CONNECTION_CLOSED/i.test(fetchErr.message)) {
                    console.warn(`[WriterLLM] 连接断开(后端已处理)，跳过重试`, fetchErr.message);
    ...
    async function callBanana2ImageAPI(prompt, options = {}) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                // 🔧 ERR_CONNECTION_CLOSED：后端已处理并扣费，不重试
                if (/ERR_CONNECTION_CLOSED/i.test(fetchErr.message)) {
                    console.warn(`[Banana2] 连接断开(后端已处理)，跳过重试`, fetchErr.message);
    ...
    async function callSora2TextToVideoAPI(prompt, options = {}) {
        ...
        // � Veo 模型（veo3.1 / veo2 / veo3）
        if (_m === 'veo3.1' || _m === 'veo2' || _m === 'veo3') {
