...
const { ConfigWizard } = require('../ui/config-wizard');
...
function reloadConfig() {
    ...
    if (orchestrator) {
        ...
        outputChannel.appendLine('[艾莉丝] 配置已重新加载');
    }
}
...
function activate(context) {
    ...
    const configWatcher = vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration('alice.aiServices') || e.affectsConfiguration('alice.safetyMode')) {
            reloadConfig();
        }
    });
    ...
    const configWizard = new ConfigWizard(context);
    ...
    context.subscriptions.push(startCommand, configureCommand, codeReviewCommand, configWatcher);
