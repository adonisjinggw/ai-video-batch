    ...
    const AgentUI = {
        ...
        _renderTeamConfig(template) {
            ...
            configArea.innerHTML = `
                ...
                <div style="color:var(--accent);font-size:13px;margin:4px 0;">🎬 起步约 ${cost} 胶片（实际按调用次数扣费，复杂任务会更多）</div>
            ...
            configArea.querySelectorAll('.at-suggest-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    const existing = goalInput.value.trim();
                    if (existing) {
                        // 已有内容：将快捷指令作为前缀，保留用户内容
                    ...
                    console.log('[AgentUI] 快捷指令已应用:', text);
                });
            });
        },
        ...
        _showResultView(result) {
            ...
            const tplId = this._selectedTemplate || '';
            ...
            resultArea.innerHTML = `
                ...
                <div class="at-deliverables">
                    ${this._renderGroupedDeliverables(deliverables, tplId)}
                </div>
        ...
        /** 智能分组渲染交付物 */
        _renderGroupedDeliverables(deliverables, tplId) {
            if (!deliverables || deliverables.length === 0) {
                return '<div style="text-align:center;padding:40px;color:#666;">暂无交付物</div>';
            }

            // 有声小说/音频制作团队：TTS音频归为"配音章节"，音乐单独展示，文本单独展示
            const isAudiobook = tplId === 'audiobook' || tplId === 'audio_production';
            if (isAudiobook) {
                return this._renderAudiobookResult(deliverables);
            }

            // 其他团队：按类型分组
            return this._renderByTypeGroups(deliverables);
        },

        /** 有声小说结果：配音章节列表 + 背景音乐 + 小说文本 */
        _renderAudiobookResult(deliverables) {
            const ttsAudios = deliverables.filter(d => d.type === 'audio' && !d.tags); // TTS无tags，音乐有tags
            const musicAudios = deliverables.filter(d => d.type === 'audio' && d.tags);
            const texts = deliverables.filter(d => d.type === 'text' || d.type === 'summary');
            const others = deliverables.filter(d => d.type !== 'audio' && d.type !== 'text' && d.type !== 'summary');

            let html = '';

            // 小说文本
            if (texts.length > 0) {
                html += `<div style="background:rgba(96,165,250,0.08);border:1px solid rgba(96,165,250,0.2);border-radius:12px;padding:16px;margin-bottom:12px;">
                    <div style="font-size:14px;font-weight:600;color:#93c5fd;margin-bottom:10px;">📖 小说内容</div>
                    ${texts.map((d, i) => this._renderDeliverable(d, i)).join('')}
                </div>`;
            }

            // 配音章节（多段TTS汇总）
            if (ttsAudios.length > 0) {
                html += `<div style="background:rgba(168,85,247,0.08);border:1px solid rgba(168,85,247,0.2);border-radius:12px;padding:16px;margin-bottom:12px;">
                    <div style="font-size:14px;font-weight:600;color:#c4b5fd;margin-bottom:10px;">🎤 配音章节（共 ${ttsAudios.length} 段）</div>
                    <div style="display:flex;flex-direction:column;gap:10px;">
                        ${ttsAudios.map((d, i) => {
                    const uid = 'tts_' + i + '_' + Date.now();
                    const label = d.description || d.agent || `第 ${i + 1} 段`;
                    return `<div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:10px;">
                                <div style="font-size:12px;color:#a78bfa;margin-bottom:6px;">🎙️ ${this._escapeHtml(label)}</div>
                                <audio id="${uid}" controls preload="auto" style="width:100%;"
                                    onerror="this.insertAdjacentHTML('afterend','<div style=\\'font-size:11px;color:#f87171;margin-top:4px;\\'>⚠️ 播放失败</div>')">
                                    <source src="${d.url}" type="audio/mpeg">
                                    <source src="${d.url}" type="audio/wav">
                                </audio>
                                <div style="margin-top:6px;display:flex;gap:8px;">
                                    <a href="${d.url}" target="_blank" style="font-size:11px;color:#a78bfa;text-decoration:none;">🔗 打开</a>
                                    <a href="${d.url}" download target="_blank" style="font-size:11px;color:#a78bfa;text-decoration:none;">⬇️ 下载</a>
                                </div>
                            </div>`;
                }).join('')}
                    </div>
            ...
            if (musicAudios.length > 0) {
                html += `<div style="background:rgba(52,211,153,0.08);border:1px solid rgba(52,211,153,0.2);border-radius:12px;padding:16px;margin-bottom:12px;">
                    <div style="font-size:14px;font-weight:600;color:#6ee7b7;margin-bottom:10px;">🎵 背景音乐</div>
                    ${musicAudios.map((d, i) => this._renderDeliverable(d, deliverables.indexOf(d))).join('')}
                </div>`;
            }

            // 其他
            if (others.length > 0) {
                html += others.map((d, i) => this._renderDeliverable(d, i)).join('');
            }

            return html || '<div style="text-align:center;padding:40px;color:#666;">暂无交付物</div>';
        },

        /** 通用按类型分组渲染 */
        _renderByTypeGroups(deliverables) {
            const groups = {
                video: { label: '🎬 视频', color: 'rgba(251,146,60,0.08)', border: 'rgba(251,146,60,0.2)', items: [] },
                image: { label: '🖼️ 图片', color: 'rgba(96,165,250,0.08)', border: 'rgba(96,165,250,0.2)', items: [] },
                audio: { label: '🎵 音频', color: 'rgba(168,85,247,0.08)', border: 'rgba(168,85,247,0.2)', items: [] },
                text: { label: '📝 文案', color: 'rgba(52,211,153,0.08)', border: 'rgba(52,211,153,0.2)', items: [] },
                summary: { label: '📋 总结', color: 'rgba(148,163,184,0.08)', border: 'rgba(148,163,184,0.2)', items: [] },
            };

            deliverables.forEach((d, i) => {
                const g = groups[d.type];
                if (g) g.items.push({ d, i });
                else groups.text.items.push({ d, i }); // 未知类型归入文案
            });

            let html = '';
            const _mergeId = '_atVideoMerge_' + Date.now();
            for (const [type, g] of Object.entries(groups)) {
                if (g.items.length === 0) continue;
                const isMulti = g.items.length > 1;
                const mergeSlot = (type === 'video' && isMulti) ? `<div id="${_mergeId}"></div>` : '';
                html += `<div style="background:${g.color};border:1px solid ${g.border};border-radius:12px;padding:16px;margin-bottom:12px;">
                    <div style="font-size:14px;font-weight:600;color:#e2e8f0;margin-bottom:10px;">${g.label}${isMulti ? `（${g.items.length} 项）` : ''}</div>
                    ${g.items.map(({ d, i }) => this._renderDeliverable(d, i)).join('')}
                    ${mergeSlot}
                </div>`;
            }

            // 延迟注入合并按钮（需要DOM已插入）
            if (groups.video.items.length >= 2 && typeof VideoMerger !== 'undefined' && VideoMerger.isSupported()) {
                const videoUrls = groups.video.items.map(({ d }) => d.url).filter(Boolean);
                if (videoUrls.length >= 2) {
                    setTimeout(() => {
                        const slot = document.getElementById(_mergeId);
                        if (slot) VideoMerger.createMergeButton(videoUrls, slot);
                    }, 100);
                }
            }

            return html || deliverables.map((d, i) => this._renderDeliverable(d, i)).join('');
        },
        ...
        viewTeamHistoryItem(id) {
            ...
            resultArea.innerHTML = `
                ...
                <div class="at-deliverables">
                    ${this._renderGroupedDeliverables(deliverables, record.teamId || '')}
                </div>
                <div class="at-result-actions">
                    <button class="at-btn at-btn-secondary" onclick="AgentUI.showTeamHistory()">← 返回列表</button>
                    <button class="at-btn at-btn-secondary" onclick="AgentUI.rerunTeamHistoryItem(${record.id})">🔄 重新运行</button>
                    <button class="at-btn at-btn-secondary" onclick="AgentUI._showView('select')">新任务</button>
                    <button class="at-btn at-btn-primary" onclick="AgentUI.close()">关闭</button>
                </div>
            `;
        ...
        /** 重新运行历史记录中的团队任务 */
        rerunTeamHistoryItem(id) {
            const history = this._loadTeamHistory();
            const record = history.find(r => r.id === id);
            if (!record) { if (typeof showToast === 'function') showToast('记录未找到'); return; }

            const tplId = record.teamId || record.templateId || '';
            // 恢复模板选择并跳到配置页
            ...
            setTimeout(() => {
                const goalInput = document.getElementById('atGoalInput');
                if (goalInput && record.goal) goalInput.value = record.goal;
            }, 150);
        },
