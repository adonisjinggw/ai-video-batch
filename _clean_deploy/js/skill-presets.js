...
(function () {
    ...
    const VIDEO_MODEL_OPTIONS = [
        { value: 'grok-video-3-15s', label: '🌟 Grok Video 3（15s 有声 推荐 12胶片）' },
    ...
    function selectVideoModel(hint = {}) {
        ...
        if (needMusic) {
            if (wantLong) return needHD ? 'wan26-1080p-15s-audio' : 'wan26-720p-15s-audio';
            return needHD ? 'wan26-1080p-5s-audio' : 'wan26-720p-5s-audio';
        }
        ...
        // 有明确时长 hint → 按时长选
        if (dur > 10) return needHD ? 'wan26-1080p-15s' : 'grok-video-3-15s';
...
难度：简单/中等/困难`;
        ...
        console.log('🧩 预置 Skills 注册完成（27 个技能）');
    }
})();
