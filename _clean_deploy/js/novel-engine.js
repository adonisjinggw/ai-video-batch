...
async function novelDeAIChapter(idx) {
    ...
    try {
        const result = await _novelLLM([
            {
                role: 'system', content: `你是一位资深${genre}小说编辑，专门负责将AI生成的小说润色为人类作家风格。你的任务是去除文本中的AI痕迹，使其读起来像真人写的网文。全文使用中文，严禁混入英文单词。

去AI化规则：
1. 删除或改写所有"仿佛""宛如""犹如""好似""恰如"等过度比喻，改用更直接的描写
2. 消除排比句和华丽辞藻堆砌，改为朴素简练的表达
3. 将书面化的对话改为口语化、有个性的说话方式，加入语气词、口头禅
4. 删除空洞的心理感悟和哲理总结，改为具体的行动和反应
5. 减少"不禁""竟然""居然""却是""倒是""微微""缓缓""淡淡"等AI高频词
6. 将过于工整的段落结构打乱，让行文节奏更自然、参差不齐
7. 场景描写精简到位，删除不推动情节的环境铺垫
8. 让角色反应更真实，少一些完美理性，多一些冲动和口误
9. 段落长短要错落有致，不要每段都差不多长
10. 保持原文情节、人物、设定完全不变，只改写表达方式

直接输出润色后的完整正文，不要加任何说明。` },
            { role: 'user', content: `/no_think\n以下是需要去AI化的章节内容：\n\n${ch.content}\n\n请对以上内容进行去AI化润色，保持情节不变，只改善表达方式使其更像真人写作。直接输出完整正文：` }
        ], { maxTokens: 16384, temperature: 0.85 });
    ...
    try {
        ...
        const content = await _novelLLM([
            ...
            onProgress: function (text) {
                document.getElementById('novelProgressLabel').textContent =
                    '第' + (_chIdx + 1) + '章 ✍️ ' + text.length + '字...';
            }
        });
    ...
    try {
        ...
        try {
            localStorage.setItem('novel_state', JSON.stringify({
                ...
                chapters: (saveData.chapters || []).map(c => ({ title: c.title, status: c.status, wordCount: c.wordCount }))
            }));
        } catch (e2) { }
