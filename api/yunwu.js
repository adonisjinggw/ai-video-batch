...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'veo3') {
            ...
            const { prompt, image_url, aspect_ratio = '16:9', model: veoModel = 'veo3.1-components-4k' } = body;
            ...
            console.log('[yunwu] Veo生成:', { model: veoModel, hasImage: !!image_url, promptLen: prompt?.length, aspect_ratio });

            // 🔒 先扣费
            if (!skipBilling && filmCost > 0 && userId) {
                const billingResult = await __billing('consume', userId, filmCost, `Veo视频:${veoModel}`);
                if (!billingResult.success && !billingResult.skipped) {
                    json(400, { success: false, error: 'BILLING_FAILED', error_code: 'BILLING_FAILED', message: billingResult.error || '扣费失败', billed: 0 });
                    return;
                }
        ...
        if (action === 'vidu') {
            ...
            console.log('[yunwu] Vidu生成:', { model_version, resolution, hasImage: !!image_url, promptLen: prompt?.length, aspect_ratio, duration });
            ...
            try {
                ...
                if (image_url) {
                    ...
                    if (image_url.startsWith('data:')) {
                        try {
                            ...
                            console.warn('[yunwu] Vidu图片上传失败:', uploadErr.message);
                        }
                    }
                    
                    // 添加图片到请求体
                    if (finalImageUrl && !finalImageUrl.startsWith('data:')) {
                        requestBody.file_infos = [{
                            type: 'Url',
                            url: finalImageUrl
                        }];
                    }
                ...
                await __saveGenerationRecord(userId, 'video', `task:${taskId}`, prompt, `vidu-${model_version}`, filmCost, { aspect_ratio, duration, resolution });
